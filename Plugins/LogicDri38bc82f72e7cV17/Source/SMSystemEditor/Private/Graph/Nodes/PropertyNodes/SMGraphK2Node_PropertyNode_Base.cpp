...
void USMGraphK2Node_PropertyNode_Base::PinDefaultValueChanged(UEdGraphPin* Pin)
{
	...
	if (USMGraphNode_Base* GraphNode = GetOwningGraphNode())
	{
		...
		{
			auto RunOptimizedConstruction = [](USMGraphNode_Base* InGraphNode)
			{
				FSMConstructionConfiguration Configuration;
				Configuration.bFullRefreshNeeded = false;
				ISMEditorConstructionManager::Get().RunAllConstructionScriptsForBlueprint(InGraphNode, MoveTemp(Configuration));
			};
			
			bool bSkipConstructionScripts = false;
			if (Pin && Pin->PinType.PinCategory == UEdGraphSchema_K2::PC_Struct)
			{
				// Fix for #228. SGameplayTagQueryGraphPin won't lookup new pins if we invalidate this from
				// editor construction scripts. This is partially an engine bug as the pin can be invalidated
				// from a normal BP setter.
				
				// TODO: This should be refactored to be more modular in case similar situations arise with different pins,
				// or to help more cases of this pin, such as triggering it from an undo/redo path.
				if (const UStruct* Struct = Cast<UStruct>(Pin->PinType.PinSubCategoryObject))
				{
					if (Struct->GetName() == FGameplayTagQuery::StaticStruct()->GetName())
					{
						// Check that the given window matches our spec. Somewhat of a hack, not a better way of
						// linking to our exact window currently.
						auto IsWindowOurWindow = [] (const SWindow& InWindow) -> bool
						{
							// Note: Localization existence is tested under LogicDriver.BugFixes.GameplayTagPinEditorName.
							FText TagEditorName;
							if (FText::FindTextInLiveTable_Advanced(TEXT("GameplayTagContainerCombo"),
								TEXT("GameplayTagQueryEntryBox_WidgetTitle"), TagEditorName))
							{
								const FText Title = InWindow.GetTitle();
								return Title.EqualTo(TagEditorName, ETextComparisonLevel::Quinary);
							}
							LDEDITOR_LOG_WARNING(TEXT("Failed to find localization for BP Pin Gameplay Tag editor."))
							return false;
						};
						
						// Verify there is a window open first. It's possible the pin could have been modified
						// programatically.
							
						TArray<TSharedRef<SWindow>> AllWindows;
						FSlateApplication::Get().GetAllVisibleWindowsOrdered(AllWindows);

						bool bIsTagWindowOpen = false;
							
						for (const TSharedRef<SWindow>& Window : AllWindows)
						{
							if (IsWindowOurWindow(*Window))
							{
								bIsTagWindowOpen = true;
								break;
							}
						}
						...
						if (!bIsTagWindowOpen)
						{
							// Reset in case somehow we didn't register the close event.
							bWaitingForWindowClose = false;
						}
						
						if (!bWaitingForWindowClose && bIsTagWindowOpen)
						{
							// Prevent all construction scripts unless we retrieve a window close event, and that window
							// is the gameplay tag editor.
							
							bWaitingForWindowClose = true;

							FSlateApplication::Get().OnWindowBeingDestroyed().AddLambda([
								this,
								RunOptimizedConstruction,
								IsWindowOurWindow,
								GraphNode](const SWindow& WindowBeingDestroyed)
							{
								if (IsWindowOurWindow(WindowBeingDestroyed))
								{
									bWaitingForWindowClose = false;
									RunOptimizedConstruction(GraphNode);
								}
							});
						}

						bSkipConstructionScripts = bWaitingForWindowClose;
					}
				}
			}

			if (!bSkipConstructionScripts)
			{
				RunOptimizedConstruction(GraphNode);
			}
		}
...
FString USMGraphK2Node_PropertyNode_Base::GetPinMetaData(FName InPinName, FName InKey)
{
	FString MetaData = Super::GetPinMetaData(InPinName, InKey);
	if (MetaData.IsEmpty())
	{
		if (UEdGraphPin* Pin = FindPin(InPinName))
		{
			// Forward the metadata request to the FProperty this node represents.
...
void USMGraphK2Node_PropertyNode_Base::ResetProperty()
{
	...
	if (ConstructionProjectSetting == ESMEditorConstructionScriptProjectSetting::SM_Standard)
	{
		FSMConstructionConfiguration Configuration;
		Configuration.bFullRefreshNeeded = false;
		ISMEditorConstructionManager::Get().RunAllConstructionScriptsForBlueprint(GraphNode, MoveTemp(Configuration));
	}
...
{
	if (!ISMEditorConstructionManager::Get().IsRunningConstructionScripts())
	{
		SetPinValueFromPropertyDefaults(false, false, true);
	}
}
...
USMPropertyGraph* USMGraphK2Node_PropertyNode_Base::GetPropertyGraph() const
{
	return Cast<USMPropertyGraph>(GetGraph());
}
